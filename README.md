# Welcome to EatRight App

This is a mobile-based application that recommends meal for the user based on their demographic profiles.

## Get started

1. Start the app

   ```bash
   npx expo start
   ```
2. Run the model backend (on backend) using python
    ```python
   -m uvicorn main:app --reload --host 0.0.0.0 --port 8000
   ```

3. Run the Supabase backend
   ```bash
   node server.js
   ```

## ONBOARDING SCREENS

<img width="280" height="630" alt="on1" src="https://github.com/user-attachments/assets/cf26ecb4-ed16-4623-b3a9-7bba47c04bf9" />
<img width="280" height="630" alt="on2" src="https://github.com/user-attachments/assets/5f6f478a-d81c-481c-9504-c8e948fb9d80" />
<img width="280" height="630" alt="on3" src="https://github.com/user-attachments/assets/467f6f26-f0ef-4f7a-ae8e-cc2c878b0c26" />

## INPUT PAGE

<img width="280" height="630" alt="input1" src="https://github.com/user-attachments/assets/9b04882c-9f58-44f8-bf8d-f3337f849ef9" />
<img width="280" height="630" alt="input2" src="https://github.com/user-attachments/assets/316b2cc8-775b-4c38-9eda-57579bf01395" />
<img width="280" height="630" alt="input3" src="https://github.com/user-attachments/assets/9487d0fb-3d92-46cb-8ac0-1a9df007355f" />

## RESULT PAGE
<img width="280" height="630" alt="result1" src="https://github.com/user-attachments/assets/137efd97-74d0-43c1-85bf-134e8f17c0dd" />
<img width="280" height="630" alt="result2" src="https://github.com/user-attachments/assets/a9d7f11d-f228-4648-a92c-e969efdfd51d" />
<img width="280" height="630" alt="result3" src="https://github.com/user-attachments/assets/d7a541ee-b264-4715-826e-e6414c5cdeed" />

## INSIGHT PAGE

<img width="280" height="630" alt="in1" src="https://github.com/user-attachments/assets/0d4b57b3-843e-4629-bc4b-2d9802bd3b2f" />








